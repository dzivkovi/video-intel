...
| A 50-min window inside a long transcript reads plausibly but matches nothing in the video; surrounding chunks are fine | **`prompt=0` on one chunk** of a chunked transcript - the per-chunk usage is logged but never inspected, so the fabricated chunk stitches in with `transcript_status: ok` | the per-chunk confab guard discards that window, marks it `FAILED (confabulation: prompt=0)` in the coverage table, and sets `transcript_status: partial`. The surrounding chunks are kept, so re-run only if the lost window matters | auto (#123) |
