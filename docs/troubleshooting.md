...
1. The pre-flight `videos.list` call (issue #70) asks for one extra part, `liveStreamingDetails`, and flags any video that has it and is not `upcoming`/`live` as a completed VOD. Same call, same quota - a scheduled premiere still takes the issue #70 not-yet-aired skip instead.
