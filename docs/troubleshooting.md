...
| A 50-min window inside a long transcript reads plausibly but matches nothing in the video; surrounding chunks are fine | **`prompt=0` on one chunk** of a chunked transcript - the per-chunk usage is logged but never inspected, so the fabricated chunk stitches in with `transcript_status: ok` | no runtime guard yet. Chunking auto-triggers above `--chunk-minutes` (default 50) on both `--url` and `--file`, so **any long-form video is exposed**. Spot-check the coverage table and a few timestamps per chunk against the video | manual (#123 open) |
