...
- `TestFallingBackToBundledIsNeverSilent` - override dirs configured but the name only exists bundled logs INFO once per name (not a WARNING, so overriding two templates does not warn about the other ten every run); deleting an override file mid-run escalates that same name to a WARNING, since it previously won from an override in this process; a second overridden name logs its own INFO line; no log at all when nothing is configured; five untouched bundled names alongside two genuinely overridden ones log exactly five INFO lines and zero WARNINGs.
...
- **Degrade, but never silently.** Malformed entries warn and drop; a missing override directory is allowed. A prompt that falls back to the bundled template while overrides are configured is the case that must NOT be silent, but the severity is graded: a name that was never resolved from an override in this process logs INFO once (routine - overriding a few templates should not warn about the rest every run), and a name that WAS resolved from an override earlier in this same run and now falls back logs WARNING instead (a real regression, not routine "not overridden" noise). The INFO memo is keyed on `(name, directory)` and the fallback log has its own memo, so an override-then-bundled switch mid-run is visible.
