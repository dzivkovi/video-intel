...
- Called at the two places the raw flag is produced: the scan pre-flight loop and `_lookup_was_livestream`. Both downstream consumers (`livestream_captions_first_applies` and `should_skip_video_mindmap_for_livestream`) read the refined value, so a premiere gets Gemini-first AND keeps its mindmap-from-video fallback.
