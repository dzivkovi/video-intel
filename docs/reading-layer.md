...
This repository turns a firehose (YouTube channels, podcasts, newsletters, a community's talk pages) into artifacts one person can actually read. The pipeline half of that story is documented elsewhere (`docs/search-internals.md`, `docs/topics-layer.md`, `docs/intelligence-layer.md`). This page is about the other half: the documents a reader opens, why they look the way they do, and how to bring your own version of them without forking the code.
...
3. **A "so what" on every item.** Each item states its implication for the reader explicitly, written against a stated reader profile. Inference is the step an interrupted reader skips, so the document does it for them.
...
- **Prompts are files.** Every template lives in `prompts/` as a self-contained markdown file with a version number in its heading. No hidden prefix assembly, no prompt built up inside code.
