...
| `description` | str | scan, transcript (single-shot, salvage, captions, chunked), mindmap, process, `backfill-descriptions` | Raw YouTube description, verbatim. Captured from the `snippet` every producer already fetches, so it costs no extra quota (issue #224). It is where a creator puts shownotes PDF links and `owner/repo` URLs, which are frequently on-screen only and therefore absent from any transcript. **Never indexed**: `_extract_video_metadata` whitelists four keys, so it cannot reach LanceDB. Filled only when absent - an existing value is never replaced. |
...
**What happens now.** Every meta writer stamps identity on first write by merging `_transcript_identity_fields(video, channel_dir)` - `video_url`, `video_id`, `channel`, `title`, `published`, and (issue #224) `description`. The last is not identity, but it travels on the same video dict and belongs to the same "YouTube-side facts this meta carries" set, so it rides the one seam every writer already merges rather than needing a sixth call site to keep in sync; the falsy-drop is exactly the semantics it needs. That covers the three transcript writers (single-shot success, salvage, captions failover), the concepts writers on both the success and failure paths (issue #129), and `cmd_mark_skip`. Falsy values are dropped so a re-stamp can only ADD identity, never downgrade a good field to empty.
