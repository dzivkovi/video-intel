...
The "Dark Factories" pattern (Compound Engineering plugin running brainstorm → plan → work → commit → review → resolve overnight) reliably produces **merge-shape PRs**: green tests, ruff clean, multi-agent code review applied, smoke evidence captured. The agent does this well. The question this doc answers is what comes *next* — how the human at the merge button decides whether the merge-shape PR is actually a merge-*ready* PR.

The receipt for this doc is [PR #55](https://github.com/dzivkovi/video-intel/pull/55) (issue #54: invert mindmap pipeline to read from on-disk transcripts instead of re-watching video). 5 hours of human validation surfaced two real bugs that 791 passing tests + a multi-persona code review had not:

1. **The `transcript_status` writer-literal mismatch.** Production code writes `"complete"` on success but the new partial-detection check tested against `"ok"` only. Every healthy single-shot transcript would have produced a mindmap stamped `mindmap_source_status: "partial"` with a `<!-- source: partial transcript (complete) -->` HTML comment. Caught when the user inspected the meta.json from the first real-input smoke. **My own unit-test fixture used `"ok"`, masking the bug at the test level.**
2. **The title-rotation orchestrator bug.** When a YouTube creator A/B-tests video titles, `cmd_mindmap` and `_cmd_process_url` computed transcript paths from the *current* API title and missed the existing transcript at the original prefix. Caught when a smoke against simonscrapes (whose title had rotated since the transcript was written) failed at Gemini's 1M-token cap. **A reviewer agent had flagged this exact pattern as a low-confidence finding (C2/C3); I dismissed it. The smoke against rotated-title content reproduced it as a hard failure.**

Both bugs would have shipped to a 720-video corpus and corrupted downstream knowledge-graph layers if the human-validation phase had been compressed to "tests pass, ship." The disciplines below are what made the catch.

## Guidance

### 1. Tests pass ≠ merge ready. Real-input smoke is the floor.

For AI-generated diffs, "tests green" reliably proves only that *the helpers compose as the test author imagined them composing*. It does not prove:
...
- The output's secondary properties (timestamp grounding, schema compatibility, cost) are within tolerances (Goodhart prompt-tuning)
- The change interacts cleanly with code paths the test author didn't think to mock
...
This is non-negotiable and complements rather than replaces the test suite. Tests verify pieces; smoke verifies emergent behavior.
...
|---|---|---|---|
| **Gate 0 — Architectural pilot** | Validate the chosen approach before committing to implementation effort | Before plan/code, when the architecture is novel | "This approach won't actually work" failures — model-specific limits, API-shape mismatches, cost-per-invocation surprises |
| **Gate 1 — Real-input smoke** | Confirm the implementation produces its primary observable signal on production-shape data | Post-implementation, post-tests-green, **before any push/merge decision** | Fixture-vs-production mismatches; integration bugs the unit suite mocked away; first-touch operator-experience issues |
| **Gate 2 — Explicit human go for destructive ops or merge** | Prevent the agent from making irreversible decisions on behalf of the human | Before any destructive operation OR the merge button, regardless of how good the upstream gates looked | Agent overconfidence; cases where the agent missed something a human would catch on a second look at the diff and smoke output together |
...
- **Gate 2:** The user held the merge button after seeing diff + smoke output together, and used the pause to ask for a third smoke (Simon) and a deeper compare against the legacy baseline. That last pause caught a regex bug in my grounding analyzer that would have understated grounding rates on solo-creator content by ~20 percentage points.
...
Systems engineering signs off in probability bands, not binary verdicts. The trained habit:
...
The discipline forces the agent to name what it doesn't know. **Tonight, the user's habit of asking for confidence intervals around grounding rates and concept-count multipliers — not just yes/no — is what kept the analysis from rubber-stamping.** The third-content-class smoke (simonscrapes) was triggered by the user noting "you've shown two data points; I want a third before I commit."
...
Tonight's pattern: before copying the new verbose mindmaps over Lenny's and Simon's existing canonical `.mindmap.md` files on G:, rename the existing files to `.mindmap.legacy.md`. The `.legacy.md` files match the resolver's wildcard fallback (`*.mindmap*.md` lexicographic-sort) so they're a gentle, useful fallback if the canonical is later deleted — not a footgun.
...
- **PR is < 30 lines and touches one well-tested helper** → tests-pass + 1-line smoke is sufficient
- **PR is a typo fix or comment update** → no smoke needed; ruff + ship
...
The systems-engineering frame — probability-based sign-off, real-input verification, adversarial cross-examination, evidence preservation — exists because billing systems, safety-critical systems, and now knowledge-graph systems share a property: the cost of a wrong-but-plausible output is far higher than the cost of a visible failure. This frame predates AI by decades. It's now the right frame for the human side of the Compound Engineering loop.
...
- The session has time budget for at least Gate 0 + Gate 1; Gate 2 is always available since it's just a pause
...
After implementation + 791 tests passing + ruff clean, the first real-input smoke ran:
...
- [PR #55](https://github.com/dzivkovi/video-intel/pull/55) — the concrete execution this learning documents (issue #54: invert mindmap pipeline).
