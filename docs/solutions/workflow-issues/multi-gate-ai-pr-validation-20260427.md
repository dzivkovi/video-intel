...
This doc exists because I did not see, until I started running Dark Factories nightly, how much engineering effort the human-validation phase actually takes. A [LinkedIn post on "Dark Factories"](https://www.linkedin.com/posts/magmainc_github-everyinccompound-engineering-plugin-share-7453892607862259712-Nf1q) captured the AI overnight chain (Compound Engineering plugin running brainstorm → plan → work → commit → review → resolve while you sleep) honestly for a small case but understated the discipline that architectural diffs into automated downstream systems actually require. The chain reliably produces merge-shape PRs: green tests, ruff clean, multi-agent code review applied, smoke evidence captured. This doc covers what comes after: how the human at the merge button decides whether the merge-shape PR is actually merge-ready.

The discipline that closes the gap is V&V (verification and validation), the systems-engineering practice I had run formally as Systems Engineering Team Lead at Derivion (2000-2004, billing platforms for global deployment, later acquired by Metavante) before the term left my daily vocabulary. At Derivion, my team's job was to validate developer-team releases before they shipped to global customers. A sign-off cycle took days to weeks per release, not minutes. Scope: integration across dependent systems, security and scalability, edge functional cases, multi-engineer probability-based sign-off (binary go/no-go was a non-starter at that scale; the team committed at confidence levels instead). Running Dark Factories daily across multiple GitHub issues brought the discipline back, at a faster cadence but with the same scope.
...
Systems engineering, codified in ISO/IEC/IEEE 15288 and elaborated by published practitioners such as Prof. Joseph E. Kasser, signs off in probability bands, not binary verdicts. The trained habit:
...
- **[ISO/IEC/IEEE 15288:2023](https://www.iso.org/standard/81702.html)**: *Systems and software engineering — System life cycle processes*. Edition 2 (current; supersedes the withdrawn 2015 edition). International standard for the life-cycle and V&V processes the gate framework above re-applies. Free abstract on the ISO page; full document is paid.
...
- **[V-model](https://en.wikipedia.org/wiki/V-model) and [V&V (verification and validation)](https://en.wikipedia.org/wiki/Verification_and_validation)**: V (verification) asks "Are we building the thing right?" V&V adds "Are we building the right thing?" The V-model is the visual structure: every level of design has a corresponding level of verification (requirements ↔ acceptance test, architecture ↔ system test, detailed design ↔ unit test). Predates Agile by decades; structural ancestor of the gate framework above.
- **Joseph E. Kasser, *[Systems Thinker's Toolbox: Tools for Managing Complexity](https://www.amazon.ca/Systems-Thinkers-Toolbox-Managing-Complexity/dp/1138606766)*** (CRC Press, 2018, 526 pages): a catalog of 100+ systems-thinking tools (lists, causal loops, templates, processes, methodologies, frameworks). Useful from either direction. Practitioners who arrived at systems thinking empirically use it to put names on patterns already in their toolkit. Readers new to the discipline use it as a reference to learn from. The book's breadth can be overwhelming on a first pass; for a curated software-focused subset, see [*Solve Complex IT Problems with Systems Thinking: Top Tools from Joseph Kasser*](https://www.youtube.com/watch?v=OlbvH2Qnb6o), a Serverless Toronto session I hosted with him.
