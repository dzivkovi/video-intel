...
> **Read the official page first; this file is the DELTA.** When this guide was written the official page did not exist, so it reconstructed the rules from creator opinion. It has since been checked against the real thing and holds up: the core (front-load, effort dial, one short instruction, verify against something concrete, ground progress claims, state boundaries, don't echo reasoning) all matches. What justifies this file's continued existence is the material Anthropic does NOT cover: model routing (sec. 1), cost mechanics (sec. 9), the two reusable templates (sec. 8), and the loops/gardening framing (sec. 12). For anything the official page does cover, fetch it rather than trusting the snapshot here.
>
> **The official page's own warning applies to this file.** It says prompts and skills built for earlier models are often too prescriptive for Fable and can degrade output. So when adapting anything below into a working prompt, cut it down. A ten-item style guide is worse than one sentence.
...
## 13. The gaps the official page filled (added 2026-07-30)

Checked this guide against the official page once it existed. Nothing above turned out wrong. Everything below is material the guide was **missing**, and all of it lands in one place: **long autonomous runs**, which is exactly how I actually use Fable (`/dark-factory`, overnight builds). Sections 1-12 were written for the interactive case and quietly under-served the unattended one. **[OFFICIAL]**

- **The early-stopping guard, and the most valuable of the four.** Deep into a long session Fable can end a turn by stating an intention ("I'll now run X") without issuing the tool call, or pause for permission it does not need. For unattended pipelines: *tell it that the user is not watching and cannot answer, that asking "Want me to...?" will block the work, that it should proceed without asking on reversible actions following from the original request, and that before ending its turn it must check its last paragraph - if that paragraph is a plan, a question, a list of next steps, or a promise about work not yet done, it does that work now with tool calls.* A stalled overnight run is the failure this prevents.
- **The anti-overplanning line, for ambiguous tasks.** Pairs with high effort: *when you have enough information to act, act; do not re-derive settled facts, re-litigate a decision already made, or narrate options you will not pursue; if weighing a choice, give a recommendation rather than an exhaustive survey - and none of this applies to thinking blocks.* Sec. 6's anti-over-engineering guard covers over-building; this one covers over-deliberating, which is a different failure.
- **The readability addendum, which matters precisely because I am asleep.** After a long unwatched run, Fable's final message is my FIRST look at anything, but by then it is writing in vocabulary it invented while working. Instruct it that the closing summary is a re-grounding rather than a continuation: outcome first, complete sentences, no arrow chains or self-coined labels, each file or flag given its own plain clause, and terms re-introduced rather than assumed. Terse shorthand between tool calls is fine; the summary is a different artifact for a different reader.
- **The context-budget reassurance.** In very long sessions Fable can offer to summarize, hand off, or start a new session, usually triggered by a harness that shows it a remaining-token countdown. If that happens: *you have ample context remaining; do not stop, summarize, or suggest a new session on account of context limits.*

Two more official items worth knowing, lower priority for me:

- **Parallel subagents.** Fable dispatches them more readily than prior models; prefer asynchronous orchestration over blocking on each return, and long-lived subagents that keep context across subtasks (cheaper via cache reads, and no bottleneck on the slowest one). The steer is one line: delegate independent subtasks and keep working, intervening only if a subagent goes off track or lacks context. Sec. 1 treats delegation as a cost tactic; officially it is also a capability.
- **Memory systems.** Fable does particularly well when it can record lessons from previous runs and reference them - one lesson per file, a one-line summary at the top, corrections and confirmed approaches alike, with the reason they mattered. I already run this shape in `~/.claude/projects/<key>/memory/`, so the news is only that it is a vendor-endorsed pattern rather than a personal habit.

**The meta-lesson, and the one that should change how I write prompts.** The official page explicitly says skills and prompts built for earlier models are often too prescriptive for Fable and can degrade its output, and recommends deleting older instructions where default behaviour is already better. My instinct has been to add rules when output disappoints. With Fable the corrective is usually the opposite: **cut the rule list, give the reason, and state the one behaviour I actually want.** Worked example: the jasmina-ops homepage-variants handoff (`work/2026-07-30/03-handoff-...md`) started as a five-rule specification and got rewritten as a creative brief with three inviolable constraints and a page of context. The rewrite was shorter and strictly better.
