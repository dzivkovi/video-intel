...
class TestPatternsAreDefinedOnce:
    ...
    def test_no_site_carries_its_own_bracketed_timestamp_literal(self):
        """Any remaining inline `\\d{1,2}:` or `\\d\\d:` bracketed literal is a
        fourth copy waiting to drift out of sync."""
        offenders = []
        for lineno, line in enumerate(SOURCE.split("\n"), 1):
            if line.strip().startswith("#"):
                continue
            if re.search(r'r?"\^?\\\[\(?\\d(\{1,2\}|\\d)', line):
...
class TestDetectOvershootMatchesItsOwnDocstring:
    ...
    @pytest.mark.parametrize("line", ["[2:05:30] x", "[02:05:30] x", "[125:30] x", "[99:30] x", "[00:00] x"])
    def test_both_helpers_agree_on_what_is_a_timestamp(self, line):
        via_extract = tv.extract_last_timestamp_seconds(line + "\n")
        via_overshoot = tv.detect_overshoot(line + "\n", 0)
        assert (via_extract is not None) == (via_overshoot is not None or via_extract == 0)


class TestOperationalSeparation:
    """The translator must not grow a video-intel dependency as a side effect
    of a shared-shape fix (the standing #129(4) precedent). `timestamp_utils`
    is the one permitted shared import, and it is stdlib-only."""
