...
class TestDetection:
    ...
    def test_rising_bursts_sort_before_cooled(self):
        rising = Burst("r", dt.date(2026, 6, 1), dt.date(2026, 6, 20), 3.0, 5, True, _events(["2026-06-01"])[0])
        cooled = Burst("k", dt.date(2026, 6, 10), dt.date(2026, 6, 15), 9.0, 5, False, _events(["2026-06-10"])[0])
        events = {
            "r": _events(CLUSTERED),
        }
        detected = detect_bursts(events, dt.date(2026, 7, 1))
        assert all(b.rising for b in detected[: sum(b.rising for b in detected)])
        assert [rising, cooled] == sorted(
            [cooled, rising], key=lambda b: (not b.rising, -b.start.toordinal(), -b.weight)
        )
