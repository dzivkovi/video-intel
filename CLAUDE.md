...
- **`process --url` chunks; `process --file` does NOT.** Issue #50 added URL chunking via Gemini's `VideoMetadata.start_offset/end_offset`. The local-file path stays single-upload because chunking it would multiply the upload cost (each chunk needs its own re-upload, defeating the "one upload" guarantee that motivated `process --file`). Reviewers: any proposal to add chunking to `process --file` needs explicit cost analysis showing the multi-upload cost is justified by the truncation rate. The current answer is "use `--start`/`--end` segments manually for over-cap local files."
- **`process --url`'s mindmap reads on-disk transcript by default; `process --file`'s mindmap watches the upload.** Issue #54 inverted the `--url` ordering (transcript → mindmap-from-transcript → concepts) and added the `mindmap_source: auto|transcript|video|none` per-channel knob. `auto` (default) routes to text-only Gemini against the on-disk transcript (~10× cheaper, no 10800-frame cap), with a fall-back to the legacy mindmap-from-video path when no transcript exists. The local-file path (`process --file`, `mindmap --file`) stays on the legacy mindmap-from-video path because chunking it would defeat the one-upload guarantee. Reviewers: any diff that promotes `source="transcript"` into `cmd_process --file` or `cmd_mindmap --file` needs the same cost-analysis pushback as the chunking proposal. Test contract is `tests/test_mindmap_from_transcript.py` (`TestCmdProcessUrlInversion`, `TestCmdScanInversion`).
