...
| "why is nothing from [creator] in my concept search", "this channel only shows up in vector search", "why does [creator] have no mindmaps" | Check that channel's `transcript_source` in `config.yaml` | A channel on the free-captions tiers (issue #225) is speech-only: no on-screen content, no speaker names. With `mindmap_source: none` it also has no mindmap and no `concepts.json`, so it is **absent from concept-mode `search`**, ranks 0 in `briefings --unseen`, and is reachable only via `search --vector`. That is the deliberate trade for not paying to watch the video, not a bug - and `transcript_source: yt-captions` on its own keeps the mindmap and concepts. |
| "what repos did [creator] link to", "what tools were in that video's description", "where is the PDF he offered", "the shownotes link", "give me the GitHub links from [creator]" | Read the `description` field in that channel's `*.meta.json` (no command) | Issue #224. A meta written from a YouTube source carries the raw description, which is where a creator puts the shownotes PDF and the `owner/repo` URLs of everything they point at - and those are frequently shown on screen without being spoken, so **no transcript contains them and `search` cannot find them**. Deliberately NOT indexed, so read meta.json directly and **parse the JSON first**: `rg` over the raw file matches the escaped newline between two links and returns one invalid joined URL.<br>`python -c "import json,glob,re,sys;[print(u) for f in sorted(glob.glob(sys.argv[1])) for u in re.findall(r'https://github\.com/[^\s)\]]+', json.load(open(f,encoding='utf-8')).get('description') or '')]" "<output_dir>/<channel>/*.meta.json"`<br>Metas written before #224 have no description; `backfill-descriptions` (curate skill) fills them. |
