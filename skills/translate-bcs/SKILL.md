...
When the video is context-heavy and captions-alone would lose meaning, run these two commands in order. Both scripts ship in the plugin's shared `scripts/` directory — there is no need to invoke another skill.

```bash
# Step 1 — generate a rich English transcript (speakers, SCREEN sections,
# On-screen text). One Gemini call that reads the video with vision enabled.
# Typical 10-minute video: 60-90 seconds.
python ${CLAUDE_SKILL_DIR}/../../scripts/video_intel.py --log-level info transcript \
  --url "https://www.youtube.com/watch?v=VIDEO_ID"
# Output: ~/video-intel/{channel_or_standalone}/{date}-{slug}.transcript.md

# Step 2 — translate that transcript to BCS, preserving structure.
# Inherits SRT_DEFAULT_THINKING_BUDGET=128 on gemini-2.5-pro.
python ${CLAUDE_SKILL_DIR}/../../scripts/translate_video.py --log-level info \
  --from-transcript "path/to/{date}-{slug}.transcript.md"
# Output: sibling .translate-bcs.txt next to the input transcript.
```

`--log-level info` is recommended so progress is visible. The default log level is `warning` which shows almost nothing during a long Gemini call. Note argparse positioning: for `video_intel.py`, `--log-level` goes **before** the subcommand; for `translate_video.py`, it can go anywhere.
